 ...
 *
 *	$Header: /home/guillaume/projects/soupcon/rrmerge/cvsdump/yabause/src/wii/wiiuse/wiiuse.c,v 1.1.2.1 2008-05-04 00:45:15 cyberwarriorx Exp $
 *
 */
...
struct wiimote_t** wiiuse_init(int wiimotes, wii_event_cb event_cb) {
	 ...
	 */
	 /*
	 Note from Robert: I have left your banner in here (with your wiiuse information), but
	 I have also added some more information/credits to avoid having to show two screens of text.
	 Is this ok with you, Para? If not, I'll change it.
	 */
	if (!g_banner) {
		printf( "\n\nSNES9xgx(0.1.1) - With Wiimote Support\n"
				"  Original Authors: Askot, dsbomb\n"
				"  Wiimote Support: Robert (www.friendcodes.com)\n"
				"  Uses Wiiuse v" WIIUSE_VERSION ". (GRRLIB Port?)\n"
				"  http://wiiuse.net  http://wiiuse.sf.net - Para\n"
				"  \n"
				"  This is just the result of me wanting to play DKC without wires, so it only\n"
				"  supports the wiimote, and you can't remap any of the buttons. I may add those\n"
				"  features later if I find the time to do.\n"
				"  \n"
				"  Controls:	  D-Pad - SNES D-Pad\n"
				"					1 - Y\n"
				"					2 - B\n"
				"				Minus - L\n"
				"				 Plus - R\n"
				...
				"			   Home+B - Menu\n"
				"  \n"
				"  To start playing, synch a wiimote by holding down 1+2 at the same time.\n"
				"  \n"
				"  Cheers,\n"
				"  Robert (www.friendcodes.com)\n"
				"  Note: Lopst has released a better version, and I'm now done with Wii homebrew.\n"
				);
		g_banner = 1;
	}
...
int wiiuse_write_data(struct wiimote_t* wm, unsigned int addr, byte* data, byte len) {
	byte buf[21] = {0};		/* the payload is always 21 */
...
int wiiuse_send(struct wiimote_t* wm, byte report_type, byte* msg, int len) {
	...
	#if defined(WIN32)
		buf[0] = report_type;
	#elif defined(GEKKO) || defined(unix)
