 ...
 *
 *	$Header: /home/guillaume/projects/soupcon/rrmerge/cvsdump/yabause/src/wii/wiiuse/events.c,v 1.1.2.1 2008-05-04 00:45:14 cyberwarriorx Exp $
 *
 */
...
static void parse_event(struct wiimote_t* wm, byte event, byte* msg);
...
#ifdef GEKKO
/* Internal routine for receiving wiimote messages via libbte */
s32 wiiuse_recv(void* arg, void* buffer, u16 len) {
	wiimote* wm = (wiimote*)arg;
	...
	if(wm && buffer && len > 0) {
		...
		/* copy the event to the event buffer */
		memcpy(&wm->event_buf, buffer, len);

		/* parse the event */
		parse_event(wm, wm->event_buf[1], wm->event_buf+2);

		/* execute the event callback if registered */
		if (wm->event_cb) {
			wm->event_cb(wm, wm->event);
		}
	} else {
		idle_cycle(wm);
	}
	...
	int evnt = 0;
		...
		for (i = 0; i < wiimotes; ++i) {
			...
			if (wiiuse_io_read(wm[i])) {
				/* parse the event */
				parse_event(wm[i], wm[i]->event_buf[0], wm[i]->event_buf+1);
		...
		for (i = 0; i < wiimotes; ++i) {
			...
			if (FD_ISSET(wm[i]->in_sock, &fds)) {
				...
				/* parse the event */
				parse_event(wm[i], wm[i]->event_buf[1], wm[i]->event_buf+2);
				evnt += (wm[i]->event != WIIUSE_NONE);
			} else {
				idle_cycle(wm[i]);
			}
